...
from lhotse.dataset import (  # noqa F401 for PrecomputedFeatures
    ...
    PrecomputedFeatures,
    SimpleCutSampler, #SingleCutSampler,
    SpecAugment,
)
...
class CommonVoiceAsrDataModule:
    ...
    @lru_cache()
    def train_es_cuts(self) -> CutSet:
          logging.info("About to get train-es cuts")
          return load_manifest_lazy(
              self.args.cv_manifest_dir / "../../es/fbank/cv-es_cuts_train.jsonl.gz"
          ) 
    ...
    @lru_cache()
    def dev_es_cuts(self) -> CutSet:
        logging.info("About to get dev-es cuts")
        return load_manifest_lazy(
    ...
    def test_full_cuts(self) -> CutSet:
        logging.info("About to get test-full cuts")
        return load_manifest_lazy(
            self.args.cv_manifest_dir / f"../../full/fbank/cv-full_cuts_test.jsonl.gz"
        )
    
    @lru_cache()
    def test_en_cuts(self) -> CutSet:
        logging.info("About to get test-en cuts")
        return load_manifest_lazy(
